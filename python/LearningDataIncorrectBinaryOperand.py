...
class LearningData(object):
    ...
    def code_to_xy_pairs_categorical(self, gen_negatives, bin_op, xs, ys, name_to_vector, type_to_vector, node_type_to_vector, code_pieces):
        left = bin_op["left"]
        right = bin_op["right"]
        operator = bin_op["op"]
        left_type = bin_op["leftType"]
    ...
    def code_to_xy_str(self, gen_negatives, bin_op, xs, ys, name_to_vector, type_to_vector, node_type_to_vector, code_pieces):
        left = bin_op["left"]
        right = bin_op["right"]
        operator = bin_op["op"]
        left_type = bin_op["leftType"]
        ...
        if gen_negatives:
            ...
            # replace one operand with the alternative one
            if replace_left:
                x_incorrect = other_operand.op + " " + right + " " + operator + " " + \
                            other_operand.type + " " + right_type + " " + \
                            parent + " " + grand_parent

            else:
                x_incorrect = left + " " + other_operand.op + " " + operator + " " + \
                            right_type + " " + other_operand.type + " " + \
                            parent + " " + grand_parent

            y_incorrect = [1]
            xs.append(str(x_incorrect))
            ys.append(y_incorrect)
            code_pieces.append(CodePiece(right, left, operator, src))    
