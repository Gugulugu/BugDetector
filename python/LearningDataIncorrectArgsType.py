...
class LearningData(object):
    ...
    # generate negatives
        if gen_negatives:
        # generate bug for incorrect argument types
        # shuffle the known type vector keys and get the first two as incorrect types
            incorrect_types = random.sample(list(type_to_vector.keys()), 2)

            # ensure incorrect types are different from original types
            while incorrect_types[0] == argument_type_strings[0] or incorrect_types[1] == argument_type_strings[1]:
                incorrect_types = random.sample(list(type_to_vector.keys()), 2)
                incorrect_type0_vector =  type_to_vector.get(
                        incorrect_types[0], [0]*type_embedding_size)
                incorrect_type1_vector =  type_to_vector.get(
                        incorrect_types[1], [0]*type_embedding_size)
                x_incorrect_types = callee_vector + argument0_vector + argument1_vector
                x_incorrect_types += base_vector + incorrect_type0_vector + incorrect_type1_vector
                x_incorrect_types += parameter0_vector + parameter1_vector  # + file_name_vector
                y_incorrect_types = [1]
                xs.append(x_incorrect_types)
                ys.append(y_incorrect_types)
                if calls != None:
                    calls.append(
                        CodePiece(callee_string, argument_strings, call["src"]))
    ...
    def code_to_xy_pairs_str(self, gen_negatives, call, xs, ys, name_to_vector, type_to_vector, classification, calls=None):
        ...
        # for all xy-pairs: y value = probability that incorrect
        x_keep = callee_string + " " + argument_strings[0] + " " + argument_strings[1]
        x_keep += " " + base_string + " " + argument_type_strings[0] + " " + argument_type_strings[1]
        x_keep += " " + parameter_strings[0] + " " + parameter_strings[1]  # + file_name_vector
        y_keep = [0]
        xs.append(x_keep)
        ys.append(y_keep)
        if calls != None:
        ...
        if gen_negatives:
            ...
            x_incorrect_types += " " + parameter_strings[0] + " " + parameter_strings[1]
            y_incorrect_types = [1]
            xs.append(x_incorrect_types)
            ys.append(y_incorrect_types)
            if calls != None:
